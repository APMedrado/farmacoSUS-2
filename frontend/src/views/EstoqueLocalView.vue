<template>
  <main>
    <div class="container">
      ...
      <div class="mb-3">
        <input type="radio" id="todoEstoque" value="todo" v-model="estoqueType" checked>
      ...
      <div class="input-group mb-3" style="max-width: 600px;" v-if="estoqueType === 'posto'">
        ...
        <p v-if="selectedPosto">Posto selecionado: {{ selectedPosto.nome }}</p>
      </div>
    ...
    fetchPosto() {
      axios.get(`http://localhost:8000/api/postos-distribuicao/${this.cnes}`)
        .then(response => {
          ...
          this.selectedPostoId = response.data.cnes;  // Use 'cnes' as the unique identifier
        })
        .catch(error => {
